...
const install = (ctx, task) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const packageNamesWithVersions = yield (0, package_manager_1.getLatestVersions)(packageNames);
    ctx.installedPackages = packageNamesWithVersions;
    let packageJson = yield (0, config_1.getPackageJson)();
    logger_1.default.debug(`Current package.json: ${(0, text_1.stringify)(packageJson)}`);
    const installGlobal = !packageJson;
    ctx.installGlobally = installGlobal;
    if (ctx.cliOptions.skipInstall) {
        if (packageJson) {
            packageJson.devDependencies = (0, object_1.sortByKeys)(Object.assign(Object.assign({}, packageJson.devDependencies), packageNamesWithVersions));
        }
        logger_1.default.debug("Skipped package installation");
        task.skip(ctx, ui.skippedInstallation);
    }
    else {
        logger_1.default.debug(`Installing packages ${(0, text_1.stringify)({ installGlobal, packageNamesWithVersions })}`);
        yield (0, package_manager_1.installPackages)(packageNamesWithVersions, { installGlobal });
        ctx.isYarn = (0, package_1.projectHasYarn)();
        if (packageJson) {
            packageJson = yield (0, config_1.getPackageJson)();
        }
    }
    if (packageJson) {
        addZeplinScripts(packageJson);
        logger_1.default.debug(`Updating package.json: ${(0, text_1.stringify)({ packageJson })}`);
        yield (0, config_1.writePackageJson)(packageJson);
