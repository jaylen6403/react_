 ...
 */
declare type DataType<T extends ApiResponse, S extends number> = T extends {
    status: S;
} ? T["data"] : never;
/**
 * Object with methods to handle possible status codes of an ApiResponse.
 */
declare type ResponseHandler<T extends ApiResponse> = {
 ...
 * })
 **/
export declare function handle<T extends ApiResponse, H extends ResponseHandler<T>>(promise: Promise<T>, handler: H): Promise<ReturnType<H[keyof H]>>;
declare const SUCCESS_CODES: readonly [200, 201, 202, 204];
declare type SuccessCodes = typeof SUCCESS_CODES[number];
declare type SuccessResponse<T extends ApiResponse> = DataType<T, SuccessCodes>;
/**
...
export declare type ApiFunction = (...args: any[]) => Promise<ApiResponse>;
