...
function updateDependencies(tree) {
    const removeOld = (0, workspace_1.updateJsonInTree)('package.json', (json, context) => {
    ...
    const packageJson = (0, workspace_1.readJsonInTree)(tree, '/package.json');
    const candidates = {
        react: '16.8.6',
        'react-dom': '16.8.6',
    ...
    const addNew = (0, workspace_1.updatePackageJsonDependencies)(updated.dependencies, updated.devDependencies);
    return (0, schematics_1.chain)([removeOld, addNew]);
}
function updateImports(host) {
    ...
    host.visit((path) => {
        if (ig.ignores((0, path_1.relative)('/', path)) || !/\.tsx?$/.test(path)) {
            return;
        }
